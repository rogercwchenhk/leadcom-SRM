          ...
          <div className="max-w-2xl mx-auto">
            ...
            <div className="mb-5">
              <div className="flex items-center gap-3 mb-5">
                <Link href="/">
                  <Button variant="ghost" size="icon" className="h-11 w-11 rounded-xl hover:bg-orange-100 bg-white shadow-sm border border-orange-100" aria-label="返回首页">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-orange-600 text-sm mt-1.5 font-medium">
                    用自然语言描述您的需求，AI 帮您处理
                  </p>
                </div>
              </div>
              ...
              {/* 步骤指示器 - 更明显 */}
              <div className="bg-white rounded-2xl p-4 shadow-sm border border-orange-100 mb-5">
                <div className="flex items-center justify-between">
                  <div className={`flex flex-col items-center gap-2 ${step === 'input' || step === 'analyzing' || step === 'review' || step === 'submitting' ? 'text-orange-600' : 'text-slate-400'}`}>
                    <div className={`w-10 h-10 rounded-2xl flex items-center justify-center text-sm font-bold shadow-sm ${step === 'input' || step === 'analyzing' || step === 'review' || step === 'submitting' ? 'bg-gradient-to-br from-orange-500 to-amber-500 text-white' : 'bg-slate-100 text-slate-500'}`}>
                      1
                    </div>
                    <span className="text-xs font-semibold">描述需求</span>
                  </div>
                  <div className={`flex-1 h-1 mx-2 rounded-full ${step === 'analyzing' || step === 'review' || step === 'submitting' ? 'bg-gradient-to-r from-orange-500 to-amber-500' : 'bg-slate-200'}`} />
                  <div className={`flex flex-col items-center gap-2 ${step === 'analyzing' || step === 'review' || step === 'submitting' ? 'text-orange-600' : 'text-slate-400'}`}>
                    <div className={`w-10 h-10 rounded-2xl flex items-center justify-center text-sm font-bold shadow-sm ${step === 'analyzing' ? 'bg-gradient-to-br from-orange-500 to-amber-500 text-white animate-pulse' : step === 'review' || step === 'submitting' ? 'bg-gradient-to-br from-orange-500 to-amber-500 text-white' : 'bg-slate-100 text-slate-500'}`}>
                      {step === 'analyzing' ? (
                    ...
                    <span className="text-xs font-semibold">AI分析</span>
                  </div>
                  <div className={`flex-1 h-1 mx-2 rounded-full ${step === 'review' || step === 'submitting' ? 'bg-gradient-to-r from-orange-500 to-amber-500' : 'bg-slate-200'}`} />
                  <div className={`flex flex-col items-center gap-2 ${step === 'review' || step === 'submitting' ? 'text-orange-600' : 'text-slate-400'}`}>
                    <div className={`w-10 h-10 rounded-2xl flex items-center justify-center text-sm font-bold shadow-sm ${step === 'submitting' ? 'bg-gradient-to-br from-orange-500 to-amber-500 text-white animate-pulse' : step === 'review' ? 'bg-gradient-to-br from-orange-500 to-amber-500 text-white' : 'bg-slate-100 text-slate-500'}`}>
                      {step === 'submitting' ? (
                        <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" />
                      ) : (
                        '3'
                    ...
                    <span className="text-xs font-semibold">确认提交</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <Card className="border-2 border-orange-100 shadow-lg bg-white">
                ...
                <CardFooter className="pb-5 pt-0 flex flex-col gap-3">
                  <Button 
                    ...
                    className="h-12 px-8 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white gap-3 text-base font-bold shadow-lg hover:shadow-xl w-full rounded-xl"
                  >
                    {step === 'analyzing' ? (
                      <>
            ...
            {(step === 'review' || step === 'submitting') && aiAnalysis && (
              <Card className="border-2 border-green-100 shadow-lg bg-white">
                ...
                <CardContent className="pb-5 pt-5">
                  <div className="space-y-6">
                    ...
                    <div className="bg-gradient-to-r from-slate-50 to-slate-100 rounded-2xl p-5 border-2 border-slate-200">
                      ...
                      <p className="text-slate-700 text-base leading-relaxed">{naturalLanguageInput}</p>
                    </div>
                ...
                <CardFooter className="pb-5 pt-0 flex flex-col sm:flex-row gap-4">
                  <Button 
                    variant="ghost" 
                    onClick={() => setStep('input')}
                    disabled={step === 'submitting'}
                    className="h-12 px-6 hover:bg-slate-100 text-slate-700 font-bold text-base flex-1 sm:flex-none border-2 border-slate-200 rounded-xl"
                  >
                    返回修改
                  </Button>
                  <Button 
                    ...
                    disabled={step === 'submitting'}
                    className="h-12 px-8 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white gap-3 text-base font-bold shadow-lg hover:shadow-xl flex-1 sm:flex-none rounded-xl"
                  >
                    {step === 'submitting' ? (
                      <>
                        <Loader2 className="h-5 w-5 animate-spin" aria-hidden="true" />
